package app.freerouting.geometry.planar

import java.math.BigInteger

/**
 * Some numerical limits and values are stored here.
 */
object Limits {

    /**
     * An upper bound (2^25) so that the product of two integers with absolut value at most CRIT_COOR is contained in the mantissa of a double with some space left for addition.
     */
    const val CRIT_INT: Int = 33554432

    /**
     * the biggest double value ( 2 ^53) , so that all integers smaller than this value are exact represented as double value
     */
    const val CRIT_DOUBLE: Double = 9007199254740992.0

    @JvmField
    val CRIT_INT_BIG: BigInteger = BigInteger.valueOf(CRIT_INT.toLong())

    @JvmField
    val sqrt2: Double = kotlin.math.sqrt(2.0)
}
